...
const ActualizarDoctor = () => {
  ...
  const doctorToUpdate = miEstado.objeto;
  console.log(doctorToUpdate.documentoIdentidad)
  const fetchUsers = async (values) => {
    try {
      await ApiRequests.putCommon(`api/Doctores/Update/${miEstado.identificador}`, values);
  ...
  const formik = useFormik({
    initialValues: {
      documentoIdentidad: doctorToUpdate.documentoIdentidad,
      genero: doctorToUpdate.genero,
      nombre: doctorToUpdate.nombre,
      apellidos: doctorToUpdate.apellidos,
      fechaNacimiento: doctorToUpdate.fechaNacimiento,
      edad: doctorToUpdate.edad,
      direccion: doctorToUpdate.direccion,
      numeroCelular: doctorToUpdate.numeroCelular,
      correo: doctorToUpdate.correo,
      especialidad: doctorToUpdate.especialidad,
      idSucursal: doctorToUpdate.idSucursal
    },
    onSubmit: values => {
      fetchUsers(values);
    },
  ...
  return (
    <form style={{ margin: '20px 50px 0 50px' }} className="row  mt-6 g-2  needs-validation" novalidate onSubmitCapture={formik.handleSubmit}>
      ...
      <hr class="border border-primary border-2 opacity-50"></hr>    <div className="col-6">
